...
def picture_test_dgl(model, X, Y, FT, essay_len, device='cpu', batch_n=1, title=False, embeddings=None, class_n=4):
    loss_function = nn.NLLLoss()
    result_list = []
    label_list = []
    # model.eval()的作用是不启用Batch Normalization和Dropout
    ...
    with torch.no_grad():
        ...
        for x, y, ft, e_len in gen:
            ...
            if title:
                # (1,9,5)
                # 使用transpose或permute进行维度变换后，调用contiguous，然后方可使用view对维度进行变形
                temp_result, pre_pred = model(inputs, pos=tp, length_essay=e_length, device=device)
                result = temp_result[:, 1:].contiguous() # result: (batch_n, doc_l-1, class_n)
                # (1,9)
                labels = labels[:, 1:].contiguous()
            else:
                result = model(inputs, pos=tp, length_essay=e_length, device=device)
